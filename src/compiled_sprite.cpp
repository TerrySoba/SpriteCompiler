 ...
 */
uint32_t compileDataLineIncrement(char* dst, uint32_t dstSize, const PixelSource& image, int16_t targetWidth)
{
    int functionSize = 0;
    int functionPos = 0;
    char lastPixel = 0;
    ...
    int lastLineIncrementFunctionSize = -1;
    bool lastLineImmediateOffsetAdded = false;
    ...
    for (int y = 0; y < image.height(); ++y)
    {
        ...
        if (y < image.height() - 1)
        {
            if (lastLineIncrementFunctionSize != functionSize)
            {
                lastLineIncrementFunctionSize = functionSize;
            ...
            {
                if (!lastLineImmediateOffsetAdded)
                {
                    // If the last line increment was already added, we need to add it again
                    functionPos -= 2;
                    functionSize -= 2;
                    if (dst)
                    {
                        memcpy(dst + functionPos, "\x81\xC3", 2); // add bx, imm16
                        uint16_t lineIncrement = targetWidth;
                        memcpy(dst + functionPos + 2, &lineIncrement, 2);
                        functionPos += 4;
                    }
                    functionSize += 4;
                }
                ...
                {
                    // just update the immediate value of the last line increment
                    if (dst)
                    {
                        uint16_t lastlineIncrement;
                        memcpy(&lastlineIncrement, dst + functionPos - 2, 2);
                        lastlineIncrement += targetWidth;
                        memcpy(dst + functionPos - 2, &lastlineIncrement, 2);
                    }
                }
                       
                lastLineImmediateOffsetAdded = true;
 ...
 * targetWidth: The width of the target screen (usually 320).
 */
uint32_t compileDataDirectOffset(char* dst, uint32_t dstSize, const PixelSource& image, int16_t targetWidth)
{
    int functionSize = 0;
    int functionPos = 0;
    char lastPixel = 0;
...
std::vector<char> CompiledSprite::compileSprite(const PixelSource& image, int16_t targetWidth)
{
    ...
    if (compiledSpriteSizeLineIncrement < compiledSpriteSizeDirectOffset)
    {
        std::vector<char> compiledSprite(compiledSpriteSizeLineIncrement);
        (void)compileDataLineIncrement(compiledSprite.data(), compiledSpriteSizeLineIncrement, image, targetWidth);
        return compiledSprite;
    }
    else
    {
        std::vector<char> compiledSprite(compiledSpriteSizeDirectOffset);
        (void)compileDataDirectOffset(compiledSprite.data(), compiledSpriteSizeDirectOffset, image, targetWidth);
        return compiledSprite;
    }
}
...
CompiledSprite::CompiledSprite(const char* filename, int16_t targetWidth)
{
    ...
    m_originalData.resize(img.width() * img.height());
